layui.use(['form','layer','layedit','laydate','upload'],function(){
    ...
    $("#menuAjax").click(function(){
        ...
        //ajax提交
        $.ajax({
        	url:"http://localhost:8080/Attendance/JurisdictionServlet?action=updateAllot",
        	type:"post",
        	data:data2,
    ...
    form.on('select(type2)', function(data){
        ...
        if(type == 3){
        	...
        	$.get("http://localhost:8080/Attendance/JurisdictionServlet?action=JurId",function (res) {
        		var result = eval('(' + res + ')');
        		var btntype = $('#btnType');
